...
The protocol team fixed this issue in the following PRs/commits:
https://github.com/DebitaFinance/Debita-V3-Contracts/commit/a60b40e8b76b7d23cff1e7ef8310819ad251f0e3
...
Internal pre-conditions
Incentive creator needs to call incentivizePair() to deposit incentive tokens for a future epoch

totalUsedTokenPerEpoch[principle][epoch] needs to be exactly 0

No users perform any lending or borrowing actions during the specified epoch

External pre-conditions
Market conditions lead to zero lending/borrowing activity during the incentivized epoch
Attack Path
Incentive creator calls incentivizePair() to set up incentives for a future epoch, transferring tokens to the contract
The epoch passes with no lending or borrowing activity
No users can claim the incentives as there are no qualifying actions (lentAmountPerUserPerEpoch and borrowAmountPerEpoch remain 0)
The tokens remain permanently locked in the contract as there is no withdrawal or recovery mechanism
Impact
...
Root Cause
Same address can be the borrower, lender and connector, there is no check against this.
DebitaV3Loan.payDebt() allows repayment of loan in the same block it was taken. (Allows the use of flash-loan to access huge funds!)
Lender can set ratio high enough to allow borrower take huge loan with dust collateral(1wei).(this reduced flash-loan needed as 1wei can be used as collateral to get unlimited principle amount reduce fees(flash-loan) and make attack more feasible/profitable).
Internal pre-conditions
Incentive is huge enough to cover attack expenses(flash-loan fees, loan disbursement fee and off-cus gasFee!)

External pre-conditions
Attacker has extra principle to cover flash-loan fees(0.05% in Aave V3)
Attack capital becomes lower when the borrow/lend inn the current epoch is low
Attack Path
Attacker takes in account the amount of incentives and total borrow/lent of the current epoch to determine profitability and also to know if there is capital(flash-loan fee).
Attacker takes flash-loan, in the flash loan call-back,
A block to the end of an epoch, creates a lend offer with HUGE ratio!(100e24 for instance) allowing borrowing huge amount with 1wei, no check/limit for this
creates a borrow offer using 1wei as collateral
calls matchOfferV3(), matching the offers, min fee of 0.2% is deducted in which 15% of it goes back to attacker, so only 0.17% net paid as fees
pays back by calling payDebt(), offcus no fee on interest since Apr is set to zero
pays back flash-loan and fees
epoch ends, and attacker claims almost all incentives(borrow + lend) in the next block(after the end of the epoch), since lent and borrow will be almost 100%, thanks to FLASH-LOAN!
Impact
...
Mitigation
Prevent repayment in the same block
It might be helpful to prevent lender == borrower == connector
Discussion
sherlock-admin2
...
Internal pre-conditions
Loan must not be already extended (extended == false)
Borrower must have waited minimum duration (10% of initial duration)
Loan must not be expired (nextDeadline() > block.timestamp)
Must have at least one unpaid offer
External pre-conditions
Current time must be close to offer's maxDeadline
maxDeadline - (block.timestamp - startedAt) - block.timestamp < 0
Attack Path
Loan starts at timestamp 1704067200 (January 1, 2024)
Time advances to 1705190400 (January 14, 2024)
Borrower attempts to extend loan
For an offer with maxDeadline 1705276800 (January 15, 2024)
...
Forces unnecessary defaults near deadline
Wastes gas on failed extension attempts
Disrupts normal loan management operationsthe
PoC
...
loanStartedAt: 1704067200 (Jan 1, 2024 00:00)
currentTime: 1705190400 (Jan 14, 2024 00:00)
maxDeadline: 1705276800 (Jan 15, 2024 00:00)
The calculation flow:

alreadyUsedTime = 1705190400 - 1704067200 = 1,123,200 (≈13 days)

extendedTime = 1705276800 - 1,123,200 - 1705190400
...
The protocol team fixed this issue in the following PRs/commits:
https://github.com/DebitaFinance/Debita-V3-Contracts/commit/dd1ad26725ba4835bc6406acf1289c1fbd33f8f2
...
Attack Path
DebitaIncentives contract owner whitelists pair of principle and collateral calling DebitaIncentives::whitelListCollateral
A user calls DebitaIncentives::incentivizePair to incentivize the already whitelisted principle.
...
Another user calls DebitaV3Aggregator::matchOffersV3 to match a previously created borrow order with one lend offer that has a non-whitelisted pair and another lend offer that has a whitelisted pair.
...
Create a file UpdateFundsTest.t.sol inside Debita-V3-Contracts/test/local/ and paste the PoC code.
...
The protocol team fixed this issue in the following PRs/commits:
https://github.com/DebitaFinance/Debita-V3-Contracts/commit/f185f42fcdbeca0bb9005767c1302c8daf24e940
